...
router.patch("/modify/:postId", async (req, res) => {
  //게시글 수정
  const { title, deadLine, category, maxMembers, contents } = req.body; // 클라이언트에서 전달 받은 수정 할 내용
  const { postId } = req.params;
  await Post.updateOne(
    { postId },
    { $set: { title, deadLine, category, maxMembers, contents } }
  );
  res.json({ success: "수정 되었습니다" });
});

// router.post("/join/:postId", async (req, res) => {
//   //참여인원 추가
//   const { postId } = req.params;
...
// router.patch("/join/:postId", async (req, res) => {
//   //참여 취소
//   const { postId } = req.params;
//   const { userName } = req.body;
